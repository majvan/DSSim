...
def _parse_args():
    ...
    parser.add_argument(
        '--scenario',
        choices=['all', 'fill-drain', 'burst', 'bounded'],
        default='all',
...
if __name__ == '__main__':
    ...
    # ---- scenario 1 --------------------------------------------------------
    if run_all or args.scenario == 'fill-drain':
        ...
        if args.with_dssim_pubsub and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSQueue',          N_EVENTS, dssim_fill_drain, N_EVENTS)
        if args.with_dssim_lite and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSLiteQueue',      N_EVENTS, lite_fill_drain, N_EVENTS)
        if run_simpy:
    ...
    # ---- scenario 2 --------------------------------------------------------
    if run_all or args.scenario == 'burst':
        print(f'\n=== Scenario 2: burst  (unlimited capacity, N={N_EVENTS:,}) ===')
        if args.with_dssim_pubsub and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSQueue     put_nowait + gget', N_EVENTS, dssim_burst_nowait_put, N_EVENTS)
            report_dssim_tq('DSSim  DSQueue     gput       + gget', N_EVENTS, dssim_burst_gput, N_EVENTS)
        if args.with_dssim_lite and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSLiteQueue put_nowait + gget', N_EVENTS, lite_burst_nowait_put, N_EVENTS)
            report_dssim_tq('DSSim  DSLiteQueue gput       + gget', N_EVENTS, lite_burst_gput, N_EVENTS)
        if run_simpy:
    ...
    # ---- scenario 3 --------------------------------------------------------
    if run_all or args.scenario == 'bounded':
        print(f'\n=== Scenario 3: bounded  (capacity=1, alternating put/get, N={N_EVENTS:,}) ===')
        if args.with_dssim_pubsub and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSQueue     gput + gget',        N_EVENTS, dssim_bounded, N_EVENTS)
        if args.with_dssim_lite and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSLiteQueue gput + gget',        N_EVENTS, lite_bounded, N_EVENTS)
        if run_simpy:
