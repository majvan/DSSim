...
def lite_many_workers(n, k):
    ...
    def producer():
        for i in range(per):
            q.put_nowait(i)
        yield from sim.gwait(0)   # yield once so consumers can run
...
def _parse_args():
    ...
    parser.add_argument(
        '--scenario',
        choices=['all', 'free-flow', 'backpressure', 'many-workers', 'blocked-getters', 'cross-notify'],
        default='all',
...
if __name__ == '__main__':
    ...
    # ---- scenario 1 --------------------------------------------------------
    if run_all or args.scenario == 'free-flow':
        print(f'=== Scenario 1: free-flow  (unbounded, 1P + 1C, N={N_EVENTS:,}) ===')
        if args.with_dssim_pubsub and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSQueue',     N_EVENTS, dssim_free_flow,  N_EVENTS)
        if args.with_dssim_lite and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSLiteQueue', N_EVENTS, lite_free_flow,   N_EVENTS)
        if run_simpy:
    ...
    # ---- scenario 2 --------------------------------------------------------
    if run_all or args.scenario == 'backpressure':
        print(f'\n=== Scenario 2: backpressure  (capacity={CAPACITY}, 1P + 1C, N={N_EVENTS:,}) ===')
        if args.with_dssim_pubsub and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSQueue',     N_EVENTS, dssim_backpressure, N_EVENTS, CAPACITY)
        if args.with_dssim_lite and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSLiteQueue', N_EVENTS, lite_backpressure,  N_EVENTS, CAPACITY)
        if run_simpy:
    ...
    # ---- scenario 3 --------------------------------------------------------
    if run_all or args.scenario == 'many-workers':
        print(f'\n=== Scenario 3: many-workers  ({N_WORKERS}P + {N_WORKERS}C, N={N_EVENTS:,}) ===')
        if args.with_dssim_pubsub and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSQueue',     N_EVENTS, dssim_many_workers, N_EVENTS, N_WORKERS)
        if args.with_dssim_lite and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSLiteQueue', N_EVENTS, lite_many_workers,  N_EVENTS, N_WORKERS)
        if run_simpy:
    ...
    # ---- scenario 4 --------------------------------------------------------
    if run_all or args.scenario == 'blocked-getters':
        print(f'\n=== Scenario 4: blocked-getters  ({N_WORKERS} getters, 1P, N={N_EVENTS:,}) ===')
        if args.with_dssim_pubsub and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSQueue',     N_EVENTS, dssim_blocked_getters, N_EVENTS, N_WORKERS)
        if args.with_dssim_lite and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSLiteQueue', N_EVENTS, lite_blocked_getters,  N_EVENTS, N_WORKERS)
        if run_simpy:
    ...
    # ---- scenario 5 --------------------------------------------------------
    if run_all or args.scenario == 'cross-notify':
        print(f'\n=== Scenario 5: cross-notify  ({CROSS_WORKERS}P + {CROSS_WORKERS}C, capacity=1, N={N_EVENTS:,}) ===')
        if args.with_dssim_pubsub and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSQueue',     N_EVENTS, dssim_cross_notify, N_EVENTS, CROSS_WORKERS, 1)
        if args.with_dssim_lite and ACTIVE_TQ_VARIANTS:
            report_dssim_tq('DSSim  DSLiteQueue', N_EVENTS, lite_cross_notify,  N_EVENTS, CROSS_WORKERS, 1)
        if run_simpy:
