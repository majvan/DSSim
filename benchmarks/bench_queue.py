...
def dssim_free_flow(n):
    ...
    sim.run()
    assert got[0] == n, f'free-flow: expected {n}, got {got[0]}'
...
def dssim_backpressure(n, capacity):
    ...
    sim.schedule(0, consumer())
    sim.run()
    assert got[0] == n, f'backpressure: expected {n}, got {got[0]}'
...
def dssim_many_workers(n, k):
    ...
    for _ in range(k):
        ...
        sim.schedule(0, consumer())
    sim.run()
    assert got[0] == n, f'many-workers: expected {n}, got {got[0]}'
...
def dssim_blocked_getters(n, k):
    ...
    sim.schedule(0, producer())
    sim.run()
    assert got[0] == n, f'blocked-getters: expected {n}, got {got[0]}'
...
def dssim_cross_notify(n, k, capacity):
    ...
    for _ in range(k):
        ...
        sim.schedule(0, producer())
    sim.run()
    assert got[0] == n, f'cross-notify: expected {n}, got {got[0]}'
