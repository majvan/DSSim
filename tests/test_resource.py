...
class TestSimResourceMixin(unittest.TestCase):
    ...
    def test3_unit_resource_factory_returns_unit_resource_instance(self):
        sim = DSSimulation()
        r = sim.unit_resource(amount=1, capacity=4)
        self.assertIsInstance(r, DSUnitResource)
        self.assertIs(r.sim, sim)
        self.assertEqual(r.amount, 1)
        self.assertEqual(r.capacity, 4)

    def test4_factories_wrong_sim_raise(self):
        sim1 = DSSimulation()
        sim2 = DSSimulation()
        with self.assertRaises(ValueError):
            sim1.resource(sim=sim2)
        with self.assertRaises(ValueError):
            sim1.unit_resource(sim=sim2)
        with self.assertRaises(ValueError):
            sim1.priority_resource(sim=sim2)


class TestUnitResource(unittest.TestCase):

    def setUp(self):
        self.sim = DSSimulation()

    def _make(self, amount=0, capacity=float('inf')):
        return DSUnitResource(amount=amount, capacity=capacity, sim=self.sim)

    def test1_non_unit_amount_raises(self):
        r = self._make(amount=1, capacity=3)
        with self.assertRaises(ValueError):
            r.get_n_nowait(2)
        with self.assertRaises(ValueError):
            r.put_n_nowait(2)

    def test2_gget_gput_roundtrip(self):
        r = self._make(amount=0, capacity=1)
        out = []

        def consumer():
            got = yield from r.gget(timeout=10)
            out.append(('got', self.sim.time, got))

        def producer():
            yield from self.sim.gwait(3)
            out.append(('put', self.sim.time, r.put_nowait()))

        self.sim.schedule(0, consumer())
        self.sim.schedule(0, producer())
        self.sim.run(20)
        self.assertEqual(out, [('put', 3, 1), ('got', 3, 1)])
...
class TestPriorityResource(unittest.TestCase):
    ...
    def test8_preemptive_interrupts_all_preempted_holders(self):
        r = self._make(amount=2, capacity=2)
        log = []
        interrupts = []

        def low(name, prio):
            got = yield from r.gget(priority=prio, preempt=True)
            self.assertEqual(got, 1)
            log.append((f'{name}_start', self.sim.time))
            try:
                yield from self.sim.gwait(20)
            except r.Preempted as exc:
                interrupts.append((name, self.sim.time, exc.amount, exc.priority))

        def high():
            yield from self.sim.gwait(3)
            got = yield from r.gget_n(amount=2, priority=1, preempt=True)
        ...
        self.sim.schedule(0, low('low_a', 10))
        self.sim.schedule(0, low('low_b', 9))
        self.sim.schedule(0, high())
        self.sim.run(20)
        ...
        self.assertEqual(
            sorted(interrupts),
            [('low_a', 3, 1, 10), ('low_b', 3, 1, 9)],
        )
        self.assertEqual(r.preempt_count, 2)
        self.assertEqual(r.preempted_amount, 2)
        self.assertEqual(r.amount, 2)


# ---------------------------------------------------------------------------
# DSResource get_cond condition helper
# ---------------------------------------------------------------------------

class TestResourceGetCond(unittest.TestCase):
    ...
    def test3_get_cond_composes_with_two_resources_via_dscircuit_without_consume(self):
    ...
    def test4_priority_get_cond_supports_preempt(self):
    ...
    def test5_get_cond_composes_with_two_resources_async_without_consume(self):
        r0 = DSResource(amount=0, capacity=1, sim=self.sim)
        r1 = DSResource(amount=0, capacity=1, sim=self.sim)
        out = []
        filters = {}
