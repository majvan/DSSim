...
# Fail on first command error and on unset variables.
# Intentionally do NOT enable pipefail: visa_check.py can fail asserts while
# the parser still validates the run output as acceptable behavior.
set -eu
...
run_py "Trackable event pipeline" "$SCRIPT_DIR/pubsub/trackable_event_pipeline.py"
