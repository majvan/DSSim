...
async def demo_filtering():
    ...
    ret = await (-f(t1) | (f(t0) & f(t2)) | (f(t2) & f(t0)))
    ...
    ret = await (-(f(t1) & f(t3)) & f(t0) & f(t2) & f(t4))
    ...
    ret = await (-(f(t6) | f(t2)) & f(t0) & f(t1) & f(t3))
    ...
    ret = await (f(lambda e:'food' in e) & f(t4) & f(t5) | f(lambda e:'drink' in e))
    ...
    ret = await (f(waiting_for_table_service(), sim=sim) | f(t0))
    ...
    time = sim.time
    t0 = sim.schedule_event(3, {'greeting': 'hello'})
    ret = await (f(t0) | f(return_apologize_after_10(), sim=sim))
