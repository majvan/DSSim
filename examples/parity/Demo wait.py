...
"""
from dssim.simulation import DSSimulation, DSAbs
...
class Prince(DSProcessComponent):
    def process(self):
        global king, kings_history, lastkingdied
        ...
        if king is None:  # there is no king, so this prince will become king, immediately
            kings_history.append(("no king", lastkingdied, self.sim.time, self.sim.time - lastkingdied))
        else:
            with self.sim.consume(king_died):  # with consume we manage that once we get notified, we consume event and noone else gets notified
                event = yield from self.sim.wait(DSAbs(self.live_till), cond=lambda e:True)  # any message will interrupt, because only king died messages are sent here
                if event is None:
                    print(self.sim.time, self, "dies before getting to the throne")
                    return
        king = self
        print(self.sim.time, self, "Vive le roi!")
        kings_history.append((self.name, self.sim.time, self.live_till, self.live_till - self.sim.time))
        yield from self.sim.wait(DSAbs(self.live_till))
