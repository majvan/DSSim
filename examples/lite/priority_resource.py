...
'''
Simple LiteLayer2 priority_resource example.
...
def run_with_autorelease_context():
    ...
    def low_with_context():
        with machine.autorelease():
            ...
            try:
                ...
                print(
                    f'{sim.time} low caught DSResourcePreempted: '
                    f'resource={exc.resource}, by={exc.by}, owner={exc.owner}, '
                    f'priority={exc.priority}, amount={exc.amount}'
                )
    ...
    # low-priority task acquires first and is preempted by high-priority task
