...
async def waiting_for_table_service():
    ...
    ret = await sim.wait(30, cond=cond)
...
async def demo_filtering():
    ...
    ret = await sim.wait(6, cond=-f(t1) | (f(t0) & f(t2)) | (f(t2) & f(t0)))
    ...
    ret = await sim.wait(10, cond=-(f(t1) & f(t3)) & f(t0) & f(t2) & f(t4))
    ...
    ret = await sim.wait(cond=f(t1) & f(t2) | f(t3))
    ...
    ret = await sim.wait(cond=f(lambda e:'food' in e) & f(t4) & f(t5) | f(lambda e:'drink' in e))
    ...
    ret = await sim.wait(cond=f(waiting_for_table_service(), sim=sim) | f(t0))
    ...
    time = sim.time
    t0 = sim.schedule_event(3, {'greeting': 'hello'})
    ret = await sim.wait(cond=f(t0) | f(return_apologize_after_10(), sim=sim))
    ...
    time = sim.time
    ret = await sim.wait(cond=f(sim.gwait(2), signal_timeout=True, sim=sim) & f(sim.gwait(6), signal_timeout=True, sim=sim) | f(sim.gwait(4), signal_timeout=True, sim=sim) & f(sim.gwait(5), signal_timeout=True, sim=sim))
