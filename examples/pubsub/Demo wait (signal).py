...
"""
from dssim import DSSimulation, DSPub, DSAgent
import random
...
class Prince(DSAgent):
    def process(self):
        ...
        with self.sim.consume(king_died):  # we consume event and noone else gets notified
            event = yield from self.sim.check_and_gwait(self.live_till - self.sim.time, cond=lambda e: king is None)  # any message will interrupt, because only king died messages are sent here
...
assert 209 <= Prince._dscomponent_instances <= 310, f"Num of kings {Prince._dscomponent_instances} is out of expected range."
