...
async def demo_filtering():
    ...
    ret = await (-f(t1) | (f(t0) & f(t2)) | (f(t2) & f(t0))).wait(6)
    ...
    ret = await (-(f(t1) & f(t3)) & f(t0) & f(t2) & f(t4)).wait(10)
    ...
    ret = await (-(f(t6) | f(t2)) & f(t0) & f(t1) & f(t3)).wait(10)
    ...
    print(ret)
    sim.cleanup()  # delete(cond=lambda e:True)
    ...
    # c.wait() subscribes for these events via the DSFuture branch.
