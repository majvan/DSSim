...
Lightweight process in LiteLayer2. Wraps a generator or coroutine with minimal overhead: no condition stacking and no join interface. Supports `abort()` for cancellation. Suitable for high-throughput scenarios without complex condition logic.
...
A named connection point on a component through which events enter or leave. From a component wiring perspective, a **publisher** is an output endpoint and a **subscriber** is an input endpoint — different vocabularies for the same concept. Hardware engineers typically speak of output/input endpoints (ports, interfaces); software engineers typically speak of publishers/subscribers. See [Chapter 3](03-pubsub-layer2.md#34-component-interfaces).
