...
async def producer(q):
    await q.put(timeout=5, "item")      # blocks until there is space
...
```python
def producer(q):
    yield from q.gput(timeout=5, "item")
...
```python
from dssim.base_components import DSLifoQueue, DSKeyQueue

q_lifo = sim.queue(policy=DSLifoQueue())                    # LIFO (stack)
q_prio = sim.queue(policy=DSKeyQueue(key=lambda e: e.priority))  # min-priority first
```

`DSKeyQueue` uses Python's `heapq` internally. Equal-priority items maintain insertion order.
...
await c.put(timeout=5, "agent_A", "agent_A", "agent_B")   # 2×A, 1×B
result = c.put_nowait("agent_A")

# Get: remove one occurrence of a specific object
obj = await c.get(timeout=5, obj_ref="agent_A")
