...
Any object with a `send(event)` method qualifies. This is an open protocol — third-party components, migrated SimPy processes, and hardware model stubs all integrate through the same interface.
...
!!! note "PubSubLayer2"
    In PubSubLayer2, `add_subscriber` requires a second argument — the delivery tier:
...
Publishers and subscribers are the natural way to define a component's interface. An input is a subscriber endpoint — something that accepts incoming events. An output is a publisher endpoint — something that sends events out.
...
To connect two components, register B's input as a subscriber on A's output publisher:
...
This composability is the core value of the publisher-subscriber principle: you can connect your component to a third-party component by subscribing to its endpoint, or expose your own endpoint so others can subscribe to it, without any adapter code.
...
| Tier | Who uses it | Semantics |
|---|---|---|
| `PRE` | Observers, loggers, probes | Always called; cannot consume the event |
| `CONSUME` | Active consumers | First to accept claims the event; rest are skipped |
| `POST_HIT` | Reaction logic | Called only when a consumer accepted |
| `POST_MISS` | Fallback logic | Called only when no consumer accepted |
...
- **CONSUME** stops at the first accepting subscriber. Later consumers in the same tier do not see the event.
...
For waiting on a single future, `task.wait(timeout)` is the preferred shorthand — it handles subscription automatically. See [Section 4.1](04-condition-filtering.md#dsprocess--wait-for-completion) for the full explanation of futures as conditions.
