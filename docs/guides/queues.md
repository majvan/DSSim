...
|---|---|---|---|
| `q.policy_for_get(amount=1, cond=AlwaysTrue, **policy_params)` | CONSUME | `q.tx_nempty` (or `q.tx_changed` if `cond` given) | Dequeues on success |
| `q.policy_for_put(*items)` | CONSUME | `q.tx_nfull` | Enqueues on success |
...
getc = q.policy_for_get(cond=lambda item: item.type == "DATA")
item = yield from getc['cond'].check_and_gwait(10)

# enqueue when space opens
putc = q.policy_for_put("ack")
...
`policy['eps']` uses endpoint metadata entries:

```python
{ep: {"tier": ep.Phase.CONSUME, "params": {"priority": 0}}}
```

**Note:** `policy_for_get(cond=...)` takes an item predicate; `policy_for_observe(cond=...)` takes a queue predicate.
