...
class _UnitWaitTimeout:
    __slots__ = ('subscriber', 'token', 'is_getter')

    def __init__(self, subscriber: ISubscriber, token: int, is_getter: bool) -> None:
        self.subscriber = subscriber
        self.token = token
...
class DSLiteResource(DSComponent, ISubscriber):
    ...
    * blocked getters/putters are resumed directly via sim.send_object().
    ...
    def _dispatch_waiters(self) -> None:
        while True:
            ...
            while self._putters:
                ...
                if self.amount + waiter.amount > self.capacity:
                    break
                self._pop_putter()
                if waiter.done:
                    continue
    ...
    def gput_n(self, timeout: TimeType = float('inf'), amount: NumericType = 1, **policy_params: Any) -> Generator[EventType, EventType, NumericType]:
        ...
        if retval > 0:
            return retval
        waiter = _Waiter(self.sim.pid, amount)
...
class DSLiteUnitResource(DSLiteResource):
    '''DSLiteResource specialization for unit-only operations.

    This resource supports only unit acquires/releases (amount=1). It keeps the
    same public API shape, but rejects non-unit amounts and uses a simpler
    dispatch loop that drains waiters by available unit count.
    '''

    @staticmethod
    def _assert_unit_amount(amount: NumericType) -> None:
        if amount != 1:
            raise ValueError('DSLiteUnitResource supports only amount=1 operations.')

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
        # Unit resource queues store subscriber identities directly (no _Waiter allocation).
        self._getters: Deque[ISubscriber] = deque()
        self._putters: Deque[ISubscriber] = deque()
        self._getter_tokens: dict[ISubscriber, int] = {}
        self._putter_tokens: dict[ISubscriber, int] = {}
        self._wait_token_seq = 0

    def _next_wait_token(self) -> int:
        self._wait_token_seq += 1
        return self._wait_token_seq

    def _enqueue_getter_pid(self, subscriber: ISubscriber) -> int:
        token = self._next_wait_token()
        self._getter_tokens[subscriber] = token
        self._getters.append(subscriber)
        return token

    def _enqueue_putter_pid(self, subscriber: ISubscriber) -> int:
        token = self._next_wait_token()
        self._putter_tokens[subscriber] = token
        self._putters.append(subscriber)
        return token

    def _handle_unit_timeout(self, event: _UnitWaitTimeout) -> None:
        subscriber = event.subscriber
        token = event.token
        if event.is_getter:
            if self._getter_tokens.get(subscriber) != token:
                return
            del self._getter_tokens[subscriber]
            try:
                self._getters.remove(subscriber)
            except ValueError:
                pass
        else:
            if self._putter_tokens.get(subscriber) != token:
                return
            del self._putter_tokens[subscriber]
            try:
                self._putters.remove(subscriber)
            except ValueError:
                pass
        self.sim.send_object(subscriber, 0)

    def send(self, event: EventType) -> EventRetType:
        if event is _DISPATCH:
            self._dispatch_scheduled = False
            self._dispatch_waiters()
        elif isinstance(event, _UnitWaitTimeout):
            self._handle_unit_timeout(event)
        elif isinstance(event, _WaitTimeout):
            self._handle_timeout(event)
        return None
    ...
    def _dispatch_waiters(self) -> None:
        while True:
            ...
            while self._getters and self.amount > 0:
                subscriber = self._getters.popleft()
                token = self._getter_tokens.pop(subscriber, None)
                if token is None:
                    continue
                self.amount -= 1
                self.sim.send_object(subscriber, 1)
                progressed = True

            while self._putters and self.amount < self.capacity:
                subscriber = self._putters.popleft()
                token = self._putter_tokens.pop(subscriber, None)
                if token is None:
                    continue
                self.amount += 1
                self.sim.send_object(subscriber, 1)
    ...
    def get_n_nowait(self, amount: NumericType = 1, **policy_params: Any) -> NumericType:
        self._assert_unit_amount(amount)
        return self.get_nowait(**policy_params)
    ...
    def gput(self, timeout: TimeType = float('inf')) -> Generator[EventType, EventType, NumericType]:
        retval = self.put_nowait()
        if retval > 0:
            return retval
        subscriber = self.sim.pid
        token = self._enqueue_putter_pid(subscriber)
        if timeout != float('inf'):
            self.sim.schedule_event(timeout, _UnitWaitTimeout(subscriber, token, is_getter=False), self)
        event = yield from self.sim.gwait()
        return 0 if event is None else event

    def gget(self, timeout: TimeType = float('inf'), **policy_params: Any) -> Generator[EventType, EventType, NumericType]:
        retval = self.get_nowait(**policy_params)
        if retval > 0:
            return retval
        subscriber = self.sim.pid
        token = self._enqueue_getter_pid(subscriber)
        if timeout != float('inf'):
            self.sim.schedule_event(timeout, _UnitWaitTimeout(subscriber, token, is_getter=True), self)
        event = yield from self.sim.gwait()
        return 0 if event is None else event

    def gput_n(self, timeout: TimeType = float('inf'), amount: NumericType = 1, **policy_params: Any) -> Generator[EventType, EventType, NumericType]:
        self._assert_unit_amount(amount)
        return (yield from self.gput(timeout=timeout))

    def gget_n(self, timeout: TimeType = float('inf'), amount: NumericType = 1, **policy_params: Any) -> Generator[EventType, EventType, NumericType]:
        self._assert_unit_amount(amount)
        return (yield from self.gget(timeout=timeout, **policy_params))
    ...
    async def put_n(self, timeout: TimeType = float('inf'), amount: NumericType = 1, **policy_params: Any) -> NumericType:
        self._assert_unit_amount(amount)
        return await self.put(timeout=timeout)

    async def get_n(self, timeout: TimeType = float('inf'), amount: NumericType = 1, **policy_params: Any) -> NumericType:
        self._assert_unit_amount(amount)
        return await self.get(timeout=timeout, **policy_params)
...
class DSLitePriorityResource(DSLiteResource):
    ...
    def _make_priority_waiter(self, amount: NumericType, priority: int = 0) -> _PriorityWaiter:
        waiter = _PriorityWaiter(self.sim.pid, amount, priority, self._prio_seq)
        self._prio_seq += 1
        return waiter
...
class SimLiteResourceMixin:
    ...
    def unit_resource(self: Any, *args: Any, **kwargs: Any) -> DSLiteUnitResource:
        sim: 'DSSimulation' = kwargs.pop('sim', self)
        if sim is not self:
            raise ValueError('The parameter sim in unit_resource() method should be set to the same simulation instance.')
        return DSLiteUnitResource(*args, **kwargs, sim=sim)
