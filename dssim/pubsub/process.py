...
class _ProcessAwaitable:
    '''Minimal awaitable used by SimProcessMixin._wait_for_event.'''
...
class DSProcess(DSFuture, SignalMixin):
    ...
    class _Starter(DSSub):
        ...
        def send(self, event: EventType) -> EventRetType:
            ...
            # _Starter.send() runs with sim.pid == _Starter. If we
...
class SimProcessMixin:
    def _gwait_for_event(self: Any, timeout: TimeType, val: EventRetType = None) -> Generator[EventType, EventType, EventType]:
        '''Core generator wait helper used by DSProcess gwait/check_and_gwait paths.'''
    ...
    async def _wait_for_event(self: Any, timeout: TimeType, val: EventRetType = None) -> EventType:
        '''Core async wait helper used by DSProcess wait/check_and_wait paths.'''
        # timeout is relative simulation time (or +inf); time_queue stores absolute times.
        if timeout == float('inf'):
            time = float('inf')
        else:
        ...
        try:
            # Pass value to the feeder and wait for next event.
            event = await _ProcessAwaitable(val)
            # Convert exception signal to a real exception.
            if isinstance(event, Exception):
                raise event
        finally:
    ...
    def gsleep(self: Any, timeout: TimeType = float('inf'), val: EventRetType = True) -> Generator[EventType, EventType, EventType]:
        retval = yield from self.pid.gsleep(timeout, val)
        return retval

    async def sleep(self: Any, timeout: TimeType = float('inf'), val: EventRetType = True) -> EventType:
        return await self.pid.sleep(timeout, val)
