...
class Container(DSStatefulComponent, SignalMixin):
    ...
    async def get1(self, timeout: TimeType = float('inf'), **policy_params: Any) -> Optional[List[EventType]]:
        ''' Simplified version of get => gets any first object '''
        with self.sim.consume(self.tx_nempty, **policy_params):
    ...
    def gget1(self, timeout: TimeType = float('inf'), **policy_params: Any) -> Generator[EventType, Optional[EventType], Optional[EventType]]:
        ''' Simplified version of gget => gets any first object; return the object '''
        with self.sim.consume(self.tx_nempty, **policy_params):
...
class Queue(DSStatefulComponent, SignalMixin):
    ...
    def get1_nowait(self, cond: CondType = AlwaysTrue) -> Optional[List[EventType]]:
        '''A version of nowait which does not reutrn a list, but rather one element.'''
    ...
    async def get1(self, timeout: TimeType = float('inf'), cond: CondType = AlwaysTrue, **policy_params: Any) -> Optional[List[EventType]]:
        '''A version of get which does not reutrn a list, but rather one element.'''
        tx = self._get_tx_endpoint(cond)
        if cond is AlwaysTrue:
    ...
    def gget1(self, timeout: TimeType = float('inf'), cond: CondType = AlwaysTrue, **policy_params: Any) -> Generator[EventType, Optional[EventType], Optional[EventType]]:
        '''A version of gget which does not reutrn a list, but rather one element.'''
        tx = self._get_tx_endpoint(cond)
        if cond is AlwaysTrue:
            check = lambda _: len(self._buffer) >= 1
        else:
...
class ContainerMixin:
    ...
    async def pop(self: Any, container: Union[Queue, Container], timeout: TimeType = float('inf'), **policy_params: Any) -> Optional[EventType]:
        try:
            retval = await container.get1(timeout, **policy_params)
        except DSAbortException as exc:
            self.scheduled_process.abort()
        return retval
    ...
    def gpop(self: Any, container: Union[Queue, Container], timeout: TimeType = float('inf'), **policy_params: Any) -> Generator[EventType, EventType, EventType]:
        try:
            retval = yield from container.gget1(timeout, **policy_params)
        except DSAbortException as exc:
            self.scheduled_process.abort()
        return retval

    def pop_nowait(self: Any, container: Union[Queue, Container]) -> EventType:
        return container.get1_nowait()
