...
class Container(DSWaitableComponent, SignalMixin):
    ...
    def put_nowait(self, *obj: EventType) -> Optional[EventType]:
        ...
        if self._available(num_items):
            ...
            self.size += num_items
            self.sim.schedule_event(0, {'event': 'container changed', 'process': self.sim.pid}, self.tx_changed)
            retval = obj
        else:
            retval = None
        return retval
...
class Queue(DSWaitableComponent, SignalMixin):
    ...
    def put_nowait(self, *obj: EventType) -> Optional[EventType]:
        ...
        if len(self) + len(obj) <= self.capacity:
            ...
            self.sim.schedule_event(0, {'event': 'queue changed', 'process': self.sim.pid}, self.tx_changed)
            retval = obj
        else:
            retval = None
        return retval
